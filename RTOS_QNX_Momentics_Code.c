...
static void *motion_task(void *arg)
{
    ...
    const double amp[NUM_JOINTS]   = { 40.0, 45.0, 35.0, 30.0, 25.0, 50.0 };
    const double freq[NUM_JOINTS]  = { 0.28, 0.20, 0.25, 0.33, 0.40, 0.12 };
    const double phase[NUM_JOINTS] = { 0.00, 1.05, 2.09, 0.52, 1.57, 3.14 };
...
seq_abort:
        ...
        /* ══════════════════════════════════════════════════════
         *  IDLE -- sine-wave telemetry
         * ══════════════════════════════════════════════════════ */
        t_idle += DT;
        for (int k = 0; k < 5; k++)
            cur[k] = 90.0
                   + amp[k] * sin(2.0 * M_PI * freq[k] * t_idle + phase[k]);
        cur[5] = 50.0
               + 50.0 * sin(2.0 * M_PI * freq[5] * t_idle + phase[5]);
